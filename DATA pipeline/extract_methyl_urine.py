...
def extract_methyl_urine(path: str, xls: pd.ExcelFile, people: pd.DataFrame) -> pd.DataFrame:
    df, from_bill = load_sheet_or_bill(path, xls, "สารMethyl")
    if "HN" not in df.columns:
        ...
        if hn_col is None and "col_2" in df.columns:
            hn_col = "col_2"
        if hn_col is None:
            raise ValueError("สารMethyl: HN column not found.")
    ...
    if value_col is None or summary_col is None:
        if from_bill:
            ...
            return out[["HN", "SCG_EmpID", "Name", "DOB", "Age", "Position", "Section", "Department", "Division", "ExamItem", "Result"]]
        raise ValueError(f"Methyl Ethyl Ketone: Cannot find required columns. Available: {cols}")
