...
def extract_xylene_urine(path: str, xls: pd.ExcelFile, people: pd.DataFrame) -> pd.DataFrame:
    sheet = "สารXylene"
    ...
    if "HN" not in df.columns:
        ...
        if hn_col is None:
            raise ValueError("สารXylene: HN column not found.")
    ...
    if "\\TL\\" in path and bill_df is not None:
        value_col = pick_col_contains_any(bill_cols, ["Xylene", "ไซลีน", "Methyl hippuric"])
        if value_col:
            ...
            def make_result(v: object) -> str:
                ...
                if num is None:
                    return ""
                status = "ผิดปกติ" if num >= 1.5 else "ปกติ"
                return f"{num},{status}"

            out = pd.DataFrame({
                "HN": bill_df["HN"],
                "ExamItem": "ตรวจสารไซลีนในปัสสาวะ",
    ...
    value_col = _find_col_norm(cols, ["Xylene", "ไซลีน", "g/g", "Cr", "creatinine"])
    summary_col = _find_col_norm(cols, ["สรุปผลการตรวจ", "สรุปผล", "ผลการตรวจ"])
    if value_col is None or summary_col is None:
        raise ValueError(f"Xylene: Cannot find required columns. Available: {cols}")
