...
const ARSENIC_KEY = "Urine Arsenic";
const ARSENIC_FALLBACK_KEYS = ["ตรวจสารหนูในปัสสาวะ"];
const GROUP_FIELDS = [
...
const getArsenicRawValue = (row: HealthRow | null) => {
  if (!row) return "";
  const primary = normalizeValue(row[ARSENIC_KEY]);
  if (primary) return primary;
  for (const key of ARSENIC_FALLBACK_KEYS) {
...
const parseArsenicValue = (value: string) => {
...
const categorizeArsenic = (value: string) => {
  if (
  ...
  const numeric = parseArsenicValue(value);
  if (numeric === null) return "other";
  return numeric > 35 ? "abnormal" : "normal";
};
...
const trendValue = (value: string) => {
  const bucket = categorizeArsenic(value);
  if (bucket === "normal") return 0.6;
  if (bucket === "abnormal") return 1;
  return null;
};

export default function UrineArsenicReport() {
  ...
  const selectedArsenic = useMemo(() => {
    if (!selectedEmpId) return null;
    const row =
      rowsByYear["2568"]?.find((item) => normalizeValue(item.SCG_EmpID) === selectedEmpId) ??
      null;
    if (!row) return null;
    const raw = getArsenicRawValue(row);
    const bucket = categorizeArsenic(raw);
    return { raw, bucket };
  ...
  const trend = useMemo(() => {
    ...
    return years.map((year) => {
      const row =
        ...
        null;
      const value = trendValue(getArsenicRawValue(row ?? null));
      return { year, value };
    });
            ...
            <div className="grid gap-4 text-sm text-gray-700">
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  <div
                    className={
                      selectedArsenic?.bucket === "normal"
                        ? "rounded-xl border border-emerald-200 bg-emerald-50 p-4"
                        : selectedArsenic?.bucket === "abnormal"
                  ...
                  >
                    ...
                    >
                      {categoryLabel(selectedArsenic?.bucket ?? "other")}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4">
              <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="text-lg font-semibold text-gray-800">
                  สัดส่วนสารหนูในปัสสาวะตามกลุ่ม
                </div>
                ...
                {groupChart.length ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={groupChart}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="normal" name="ปกติ" fill="#16A34A" stackId="arsenic" />
                      <Bar dataKey="abnormal" name="ผิดปกติ" fill="#DC2626" stackId="arsenic" />
                      <Bar dataKey="notTested" name="ไม่ได้รับการตรวจ" fill="#6B7280" stackId="arsenic" />
                      <Bar dataKey="other" name="อื่นๆ" fill="#94A3B8" stackId="arsenic" />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
